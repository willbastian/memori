...
CREATE TABLE gate_template_approvals (
    template_id TEXT NOT NULL,
    version INTEGER NOT NULL CHECK(version > 0),
    approved_at TEXT NOT NULL,
    approved_by TEXT NOT NULL,
    PRIMARY KEY(template_id, version),
    FOREIGN KEY(template_id, version) REFERENCES gate_templates(template_id, version)
);
...
FROM gate_templates
WHERE created_by NOT LIKE 'llm:%'
  AND definition_json LIKE '%"command"%';
...
CREATE TABLE events (
    ...
    entity_type TEXT NOT NULL CHECK(entity_type IN ('issue','session','packet','focus','gate_template','gate_set')),
