...
CREATE TABLE events (
    ...
    entity_type TEXT NOT NULL CHECK(entity_type IN ('issue','session','packet','focus')),
    entity_id TEXT NOT NULL,
    entity_seq INTEGER NOT NULL CHECK(entity_seq > 0),
    event_type TEXT NOT NULL CHECK(event_type IN ('issue.created','issue.updated','issue.linked','gate.evaluated','session.checkpointed','packet.built','focus.used')),
