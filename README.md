...
If the repository uses `AGENTS.md` for collaborator guidance, add `--append-agents-md` during init to append Memori's standard non-interactive agent setup, worktree continuity guidance, and managed "Land The Plane" closeout checklist in the current repo. The managed block is idempotent, so rerunning the command updates that block instead of duplicating it.
...
If an agent skips `MEMORI_PRINCIPAL=llm`, Memori treats the caller as a human writer and mutating commands will prompt for the configured human password instead.
...
If you bootstrap agent guidance with `memori init --append-agents-md`, the generated `AGENTS.md` block includes the same non-interactive agent identity setup and worktree continuity workflow, so future agents know to declare `MEMORI_PRINCIPAL=llm` before mutations and to adopt or register the workspace before relying on resume or board context.
